...
OUT_DIR = "dual_domain_soft_scaled_u1_fno_scaled_icbc_results_example1"
...
def source_f_torch(x: torch.Tensor, t: torch.Tensor, alpha: float = ALPHA) -> torch.Tensor:
    """
    Example 1 source term from the paper:
    f = u_t + (-Delta)^{alpha/2} u
    ...
    frac_part = (
        ...
        - 3.0 * g5 * (x ** (4 - alpha) + (1 - x) ** (4 - alpha))
        + 3.0 * g6 * (x ** (5 - alpha) + (1 - x) ** (5 - alpha))
        - g7 * (x ** (6 - alpha) + (1 - x) ** (6 - alpha))
    ) / denom

    return u0_torch(x) + (t + 1.0) * frac_part
...
def true_u1_inverse_fourier(x: torch.Tensor, t: torch.Tensor):
    ...
    decay = torch.exp(-C * (kk ** ALPHA) * tt)

    real = decay * real0_grid.reshape(1, Nk)
    imag = decay * imag0_grid.reshape(1, Nk)
