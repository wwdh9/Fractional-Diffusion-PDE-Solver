...
空间观察区间 `[0,1]`，200 个空间点；T=1，M=80，dt=0.0125；预测频率截断 Kmax=8、逆变换节点 80。c=1，随机种子 0，网络采用 float32。
...
进入 `example1` 目录后执行以下命令。已验证环境为 Python 3.11、PyTorch 2.5.1+cu121、RTX 3060 Laptop 6 GB。CPU 运行耗时更长。
...
|---:|---:|---:|---:|---:|---:|---:|
| 0.4 | 4.337181e-04 | 4.743928e-06 | 4.318243e-04 | 4.237511e-02 | 1.918798e-03 | 9.447609e-05 |
| 0.8 | 1.979392e-04 | 1.230304e-05 | 2.065068e-04 | 2.791045e-02 | 1.919357e-03 | 2.015909e-05 |
| 1.2 | 8.631664e-05 | 2.660223e-05 | 1.117233e-04 | 1.906925e-02 | 1.920590e-03 | 3.475351e-05 |
| 1.6 | 4.519004e-05 | 6.647948e-05 | 9.786462e-05 | 1.426157e-02 | 1.917781e-03 | 8.505120e-05 |
| 2.0 | 3.141692e-05 | 4.262273e-06 | 3.334498e-05 | 1.131503e-02 | 1.976011e-03 | 8.788333e-06 |
...
α=1.6 的总解全时空 MAE 为 `9.786462e-05`，t=1 的 MAE 为 `8.505120e-05`；最大点误差为 `1.917781e-03`，它不等同于平均误差。 初始时刻总解 MAE 为 `1.158233e-03`；u2 初值为零，初值误差集中在 u1 的重构。 按分支 MAE 比较，FNO/u2 较大；总解误差由两支误差相加得到，可能出现局部抵消。
