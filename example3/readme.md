...
空间观察区间 `[-1,1]`，200 个空间点；T=1，M=80，dt=0.0125；预测频率截断 Kmax=8、逆变换节点 80。c=1，随机种子 0，网络采用 float32。
...
进入 `example3` 目录后执行以下命令。已验证环境为 Python 3.11、PyTorch 2.5.1+cu121、RTX 3060 Laptop 6 GB。CPU 运行耗时更长。
...
|---:|---:|---:|---:|---:|---:|---:|
| 0.4 | 1.831580e-02 | 1.407176e-03 | 1.801671e-02 | 2.110782e-02 | 3.961723e-01 | 4.008619e-03 |
| 0.8 | 6.880178e-03 | 3.276736e-03 | 9.468455e-03 | 1.113136e-02 | 2.616353e-01 | 6.680289e-03 |
| 1.2 | 2.334168e-03 | 1.090353e-03 | 3.053683e-03 | 5.844372e-03 | 1.546157e-01 | 1.888044e-03 |
| 1.6 | 1.346090e-03 | 2.688778e-03 | 3.458013e-03 | 5.938880e-03 | 8.480986e-02 | 4.897474e-03 |
| 2.0 | 1.338234e-03 | 1.378437e-03 | 1.725840e-03 | 4.744553e-03 | 5.378872e-02 | 1.925882e-03 |
...
α=1.6 的总解全时空 MAE 为 `3.458013e-03`，t=1 的 MAE 为 `4.897474e-03`；最大点误差为 `8.480986e-02`，它不等同于平均误差。 初始时刻总解 MAE 为 `1.757422e-02`；u2 初值为零，初值误差集中在 u1 的重构。 按分支 MAE 比较，FNO/u2 较大；总解误差由两支误差相加得到，可能出现局部抵消。

本组固定配置的总解 MAE 在 α=2.0 时最小、α=0.4 时最大。这是各 α 的一次实际运行结果，不能据此假定误差随 α 单调变化。
