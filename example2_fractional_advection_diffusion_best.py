...
# ------------------------------------------------------------
# 1D Example 2: fractional advection-diffusion
#   u_t + c (-Delta)^{alpha/2} u + nu*u_x = f
#   u(x,t) = x^3(1-x)^3(t+1), x in [0,1]
#
# New framework:
...
# Add full-solution IC/BC losses during FNO training.
...
OUT_DIR = "dual_domain_soft_scaled_u1_fno_scaled_icbc_results_example2"
...
def source_f_torch(
    ...
    g4 = math.gamma(4) / math.gamma(4-alpha)
    g5 = math.gamma(5) / math.gamma(5-alpha)
    g6 = math.gamma(6) / math.gamma(6-alpha)
    g7 = math.gamma(7) / math.gamma(7-alpha)

    denom = 2.0 * math.cos(math.pi*alpha/2.0)

    frac_part = (
        g4*(x**(3-alpha)+(1-x)**(3-alpha))
        -3*g5*(x**(4-alpha)+(1-x)**(4-alpha))
        +3*g6*(x**(5-alpha)+(1-x)**(5-alpha))
        -g7*(x**(6-alpha)+(1-x)**(6-alpha))
    ) / denom


    # u_t
    time_part = u0_torch(x)


    # nu*u_x
    advection_part = NU*(t+1.0)*(
        3*x**2*(1-x)**3
        -
        3*x**3*(1-x)**2
    )
...
def true_u1_inverse_fourier(x: torch.Tensor, t: torch.Tensor):
    """
    Reference solution of u1:
        u1_hat(k,t) = exp(-C * |k|^ALPHA * t) * u0_hat(k)

    Then reconstruct by the same numerical inverse Fourier rule.

    Memory-safe version:
        u0_hat is computed only once on K_GRID, not on repeated kk_flat.
    """
    B = x.numel()
    Nk = K_GRID.numel()

    real0_grid, imag0_grid = u0_hat_torch(K_GRID)

    kk = K_GRID.reshape(1, Nk)
    tt = t.reshape(B, 1)

    complex_decay = torch.exp(
        -(C * (kk ** ALPHA) + 1j * NU * kk) * tt
    )

    u0_complex = (
        real0_grid.reshape(1, Nk)
        + 1j * imag0_grid.reshape(1, Nk)
    )

    u_complex = complex_decay * u0_complex

    real = torch.real(u_complex)
    imag = torch.imag(u_complex)

    phase = x.reshape(B, 1) * kk

    # Re[(a+ib)(cos+i sin)] = a cos - b sin
    integrand = real * torch.cos(phase) - imag * torch.sin(phase)

    u1_true = torch.trapz(integrand, K_GRID, dim=1) * math.sqrt(2.0 / math.pi)
    return u1_true
...
def u0_hat_torch(k: torch.Tensor, quad_n: int = 512):
    """
    Compute Fourier transform of u0 on [0,1]:
        \hat u0(k)=1/sqrt(2pi) int_0^1 u0(x) exp(-i k x) dx

    Return:
        real, imag
    """
    xq = torch.linspace(0.0, 1.0, quad_n, device=k.device)
    uq = u0_torch(xq)
...
def fractional_laplacian_gl_reconstructed(
    ...
    for m in range(GL_TERMS):
        ...
        mask_left = ((x_left >= 0.0) & (x_left <= 1.0)).float()
        mask_right = ((x_right >= 0.0) & (x_right <= 1.0)).float()

        # 即使 x 超出 [0,1]，reconstruct 也能算；随后乘 mask 做零延拓
...
def physical_pde_loss_u1(net: nn.Module):
    ...
    eps = 2.0 * GL_H
    x = eps + (1.0 - 2.0 * eps) * torch.rand(B_PHYS, device=device)
...
def physical_ic_loss_u1(net: nn.Module):
    ...
    """
    x = torch.rand(B_IC, device=device)
...
class FNO1dNoBeta(nn.Module):
    ...
    def forward(self, f_input):
        ...
        x_grid = torch.linspace(0.0, 1.0, size_x, device=f_input.device)
        x_grid = x_grid.reshape(1, size_x, 1).repeat(batchsize, 1, 1)

        t_grid = torch.linspace(0.0, T_END, batchsize + 1, device=f_input.device)[:-1]
...
def build_training_grids(net_u1):
    ...
    """
    x_grid = torch.linspace(0.0, 1.0, S, device=device)
...
def train_fno_branch(net_u1):
    """
    Train the FNO branch for u2.

    Original scaled FNO logic is preserved:
        f_input = f_train / F_SCALE
        g = G_SCALE * FNO(f_input)
        u2_pred = cumsum(g, dim=0) * dt

    Added FNO IC/BC loss:
        u_pred_all = u1_all.detach() + u2_all
        L_ic = MSE((u_pred_all[0]-u_true_all[0]) / U_FULL_SCALE)
        L_bc = MSE(u_pred_all[:,0] / U_FULL_SCALE)
             + MSE(u_pred_all[:,-1] / U_FULL_SCALE)
    """
    ...
    # ------------------------------------------------------------
    u2_target = (u_true - u1_pred.detach()).detach()  # [M,S]
    ...
    for ep in range(1, EPOCH_FNO + 1):
        ...
        if USE_FNO_ICBC_LOSS:
            ...
            loss_bc_left = torch.mean((u_pred_all[:, 0] / u_full_scale) ** 2)
            loss_bc_right = torch.mean((u_pred_all[:, -1] / u_full_scale) ** 2)
...
def evaluate_and_plot(net_u1, fno_model, data_pack):
    ...
    with torch.no_grad():
        ...
        g_norm = fno_model(f_input).squeeze(-1)      # [M, S]
...
def main():
    print("Training u1 branch ...")
    print("u1 branch: soft scaled PINN")
    print("  uhat_theta(k,t) = s * NN(k,t)")
    print("  NO hard IC ansatz")
    print(f"  USE_OUTPUT_SCALE = {USE_OUTPUT_SCALE}")
    print(f"  USE_NORMALIZED_FREQ_LOSS = {USE_NORMALIZED_FREQ_LOSS}")
    print(f"  USE_NORMALIZED_PHYS_IC = {USE_NORMALIZED_PHYS_IC}")
    print(f"  USE_FNO_ICBC_LOSS = {USE_FNO_ICBC_LOSS}, W_FNO_IC = {W_FNO_IC}, W_FNO_BC = {W_FNO_BC}")
    net_u1, pinn_history = train_u1_branch()

    print("\nTraining FNO branch without beta, with FNO IC/BC losses ...")
    fno_model, fno_history, data_pack = train_fno_branch(net_u1)

    evaluate_and_plot(net_u1, fno_model, data_pack)
    plot_losses(pinn_history, fno_history)

    torch.save(net_u1.state_dict(), os.path.join(OUT_DIR, "u1_soft_scaled_pinn.pt"))
    torch.save(fno_model.state_dict(), os.path.join(OUT_DIR, "fno_scaled_no_beta_icbc.pt"))
    print("Saved models to:", OUT_DIR)
    debug_network_k0(net_u1)
    plot_freq_ic_check(net_u1)
